...
export type DropDownOptionGenerator = () => DropDownOption[];

// Display a text searchable list of options.
// The list of options can be static (passes thorough the
// `options` argument) or dynamically generated by a
// function passed by the `optionsGenerator` parameter
export class DropDownOptionsMenu extends HTMLDivElement {
  private options: HTMLOptionElement[];
  private optionsGenerator: undefined | DropDownOptionGenerator;
  ...
  constructor(
    ...
    optionsGenerator?: DropDownOptionGenerator
  ) {
    super();
    this.id = id;
  ...
  private _initHTMLElement(options: undefined | DropDownOption[]) {
  ...
  private _checkTextInput() {
    ...
    if (this.optionsGenerator !== undefined) {
      for (let option of this.optionsGenerator()) {
        const optionElement = document.createElement("option");
        optionElement.value = option.value;
        optionElement.innerHTML = option.name;
        optionElement.className = "drop-down-option";
