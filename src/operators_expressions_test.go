...
func Test_Assignements(t *testing.T) {
	...
	// This test declares variables with ':=', which fails if the name is
	// already declared. Other tests share the package-level VM and declare
	// some of the same names, so use a private one here.
	be := new(ByteEater).InitVM()
	...
	source = `
	l := [1.1, 2.02, 3.003, 4.0004, 5.00005, 6.000006, 7.00000007, 8.000000008, 9.0000000009, 10.00000000010]
...
// Test_Not_List covers issue #7: `not` in front of a list.
//
// A list is built at run time from the elements the compiler pushed on the
// stack, and its length is carried by OP_END_LIST. When that length was
// missing every list came out empty, so the operand of `not` resolved to nil
// and the unary path crashed instead of negating the list.
func Test_Not_List(t *testing.T) {
	vm := new(ByteEater).InitVM()

	vm.RunSource(`not [true, false, true]`)
	if err := checkCurrentResult(vm, []bool{false, true, false}); err != nil {
		t.Error(err)
	}

	// A list must survive on its own, too.
	vm.RunSource(`[true, false, true]`)
	if err := checkCurrentResult(vm, []bool{true, false, true}); err != nil {
		t.Error(err)
	}

	// A list of lists has no series representation, so `not` cannot apply.
	// It must report that rather than crash: reporting the unsupported
