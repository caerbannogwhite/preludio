...
func CompileFile(path string) ([]byte, []meta.LogEnty, error) {
...
func (bf *ByteFeeder) RemoveLastInstruction() {
	bf.instructions = bf.instructions[:len(bf.instructions)-meta.PRELUDIO_INSTRUCTION_SIZE]
	bf.lastInstruction = meta.OPCODE(bf.instructions[len(bf.instructions)-meta.PRELUDIO_INSTRUCTION_SIZE])
}

func (bf *ByteFeeder) GetBytecode() ([]byte, []meta.LogEnty, error) {
	...
	if bf.err == nil {
		bf.logs = append(bf.logs, meta.LogEnty{
			LogType: meta.LOG_DEBUG,
			Level:   5,
...
func (bf *ByteFeeder) EnterStmt(ctx *StmtContext) {
	bf.AppendInstruction(meta.OP_START_STMT, 0, 0)
}

// // ExitStmt is called when production stmt is exited.
func (bf *ByteFeeder) ExitStmt(ctx *StmtContext) {
	bf.AppendInstruction(meta.OP_END_STMT, 0, 0)
}
...
func (bf *ByteFeeder) ExitVarAssignStmt(ctx *VarAssignStmtContext) {
	...
	bf.AppendInstruction(meta.OP_VAR_ASSIGN, 0, pos)
}
...
func (bf *ByteFeeder) ExitVarDeclStmt(ctx *VarDeclStmtContext) {
	pos := bf.symbolTable.Add(ctx.IDENT().GetSymbol().GetText())
	bf.AppendInstruction(meta.OP_VAR_DECL, 0, pos)
}
...
func (bf *ByteFeeder) EnterFuncCall(ctx *FuncCallContext) {
	bf.AppendInstruction(meta.OP_START_FUNC_CALL, 0, 0)
}
...
func (bf *ByteFeeder) ExitFuncCall(ctx *FuncCallContext) {
	...
	bf.AppendInstruction(meta.OP_MAKE_FUNC_CALL, 0, bf.symbolTable.Add(ctx.IDENT().GetSymbol().GetText()))
}
...
func (bf *ByteFeeder) ExitFuncCallParam(ctx *FuncCallParamContext) {
	bf.AppendInstruction(meta.OP_END_CHUNCK, 0, 0)
}
...
func (bf *ByteFeeder) ExitNamedArg(ctx *NamedArgContext) {
	...
	pos := bf.symbolTable.Add(ctx.IDENT().GetSymbol().GetText())
	bf.AppendInstruction(meta.OP_PUSH_NAMED_PARAM, 0, pos)
}
...
func (bf *ByteFeeder) ExitAssign(ctx *AssignContext) {
	...
	pos := bf.symbolTable.Add(ctx.IDENT().GetSymbol().GetText())
	bf.AppendInstruction(meta.OP_PUSH_ASSIGN_IDENT, 0, pos)
}
...
func (bf *ByteFeeder) ExitExpr(ctx *ExprContext) {
	...
	case 2:
		...
		case meta.SYMBOL_UNARY_SUB:
			if bf.lastInstruction == meta.OP_UNARY_SUB {
				bf.RemoveLastInstruction()
				return
			}
			bf.AppendInstruction(meta.OP_UNARY_SUB, 0, 0)

		case meta.SYMBOL_UNARY_ADD:
			if bf.lastInstruction == meta.OP_UNARY_ADD {
				return
			}
			bf.AppendInstruction(meta.OP_UNARY_ADD, 0, 0)

		case meta.SYMBOL_UNARY_NOT:
			bf.AppendInstruction(meta.OP_UNARY_NOT, 0, 0)
		}
	...
	case 3:
		...
		case meta.SYMBOL_IN:
			bf.AppendInstruction(meta.OP_BINARY_IN, 0, 0)

		case meta.SYMBOL_BINARY_MUL:
			bf.AppendInstruction(meta.OP_BINARY_MUL, 0, 0)

		case meta.SYMBOL_BINARY_DIV:
			bf.AppendInstruction(meta.OP_BINARY_DIV, 0, 0)

		case meta.SYMBOL_BINARY_MOD:
			bf.AppendInstruction(meta.OP_BINARY_MOD, 0, 0)

		case meta.SYMBOL_BINARY_EXP:
			bf.AppendInstruction(meta.OP_BINARY_EXP, 0, 0)

		case meta.SYMBOL_BINARY_ADD:
			bf.AppendInstruction(meta.OP_BINARY_ADD, 0, 0)

		case meta.SYMBOL_BINARY_SUB:
			bf.AppendInstruction(meta.OP_BINARY_SUB, 0, 0)

		case meta.SYMBOL_BINARY_EQ:
			bf.AppendInstruction(meta.OP_BINARY_EQ, 0, 0)

		case meta.SYMBOL_BINARY_NE:
			bf.AppendInstruction(meta.OP_BINARY_NE, 0, 0)

		case meta.SYMBOL_BINARY_GE:
			bf.AppendInstruction(meta.OP_BINARY_GE, 0, 0)

		case meta.SYMBOL_BINARY_LE:
			bf.AppendInstruction(meta.OP_BINARY_LE, 0, 0)

		case meta.SYMBOL_BINARY_GT:
			bf.AppendInstruction(meta.OP_BINARY_GT, 0, 0)
		...
		case meta.SYMBOL_BINARY_OR:
			bf.AppendInstruction(meta.OP_BINARY_OR, 0, 0)

		case meta.SYMBOL_BINARY_MODEL:
			bf.AppendInstruction(meta.OP_BINARY_MODEL, 0, 0)

		case meta.SYMBOL_BINARY_COALESCE:
			bf.AppendInstruction(meta.OP_BINARY_COALESCE, 0, 0)
		}
	}
}
...
func (bf *ByteFeeder) ExitLiteral(ctx *LiteralContext) {
	// IDENT
	if ctx.IDENT() != nil {
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_SYMBOL,
			bf.symbolTable.Add(
	...
	// NULL
	if ctx.NA() != nil {
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_NULL, 0)
	} else

	// BOOLEAN
	if ctx.BOOLEAN_LIT() != nil {
		if ctx.BOOLEAN_LIT().GetSymbol().GetText() == meta.SYMBOL_TRUE {
			bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_BOOLEAN, 1)
		} else {
			bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_BOOLEAN, 0)
		}
	} else

	// INTEGER
	if ctx.INTEGER_LIT() != nil {
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_INTEGER,
			bf.symbolTable.Add(
	...
	if ctx.RANGE_LIT() != nil {
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_RANGE,
			bf.symbolTable.Add(
	...
	if ctx.FLOAT_LIT() != nil {
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_FLOAT,
			bf.symbolTable.Add(
	...
	if ctx.STRING_LIT() != nil {
		...
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_STRING,
	...
	if ctx.STRING_INTERP_LIT() != nil {
		...
		strBegin := 0
		exprBegin := strings.Index(val, meta.SYMBOL_LBRACE)
		exprEnd := -1
		exprNum := 0
		for exprBegin != -1 {

			exprEnd = strings.Index(val, meta.SYMBOL_RBRACE)
			if exprEnd == -1 {
	...
	if ctx.STRING_RAW_LIT() != nil {
		...
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_STRING_RAW,
	...
	if ctx.STRING_PATH_LIT() != nil {
		...
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_STRING_PATH,
			bf.symbolTable.Add(val[2:len(val)-1]))
	} else
	...
	if ctx.REGEX_LIT() != nil {
		...
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_REGEX,
			bf.symbolTable.Add(val[2:len(val)-1]))
	} else
	...
	if ctx.DATE_LIT() != nil {
		...
		bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_DATE,
			bf.symbolTable.Add(val[2:len(val)-1]))
	} else

	// DURATION
	if ctx.DURATION_LIT() != nil {
		val := strings.Split(ctx.DURATION_LIT().GetText(), meta.SYMBOL_COLON)
		switch val[1] {
		case meta.SYMBOL_DURATION_MICROSECOND, meta.SYMBOL_DURATION_MICROSECOND_SHORT:
			bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_DURATION_MICROSECOND,
				bf.symbolTable.Add(val[0]))

		case meta.SYMBOL_DURATION_MILLISECOND, meta.SYMBOL_DURATION_MILLISECOND_SHORT:
			bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_DURATION_MILLISECOND,
				bf.symbolTable.Add(val[0]))

		case meta.SYMBOL_DURATION_SECOND, meta.SYMBOL_DURATION_SECOND_SHORT:
			bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_DURATION_SECOND,
				bf.symbolTable.Add(val[0]))

		case meta.SYMBOL_DURATION_MINUTE, meta.SYMBOL_DURATION_MINUTE_SHORT:
			bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_DURATION_MINUTE,
				bf.symbolTable.Add(val[0]))

		case meta.SYMBOL_DURATION_HOUR, meta.SYMBOL_DURATION_HOUR_SHORT:
			bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_DURATION_HOUR,
				bf.symbolTable.Add(val[0]))

		case meta.SYMBOL_DURATION_DAY, meta.SYMBOL_DURATION_DAY_SHORT:
		...
		case meta.SYMBOL_DURATION_YEAR, meta.SYMBOL_DURATION_YEAR_SHORT:
			bf.AppendInstruction(meta.OP_PUSH_TERM, meta.TERM_DURATION_YEAR,
				bf.symbolTable.Add(val[0]))
		}
	}
}
...
func (bf *ByteFeeder) EnterList(ctx *ListContext) {
	bf.AppendInstruction(meta.OP_START_LIST, 0, 0)
}
