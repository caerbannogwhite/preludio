...
func Test_Lists_Expressions(t *testing.T) {
	...
	bytecode, _, _ = bytefeeder.CompileSource(`false and [false, true, false]`)
	be.RunBytecode(bytecode)
	if err = checkCurrentResult(be, []bool{false, false, false}); err != nil {
		t.Error(err)
	}

	bytecode, _, _ = bytefeeder.CompileSource(`[false, true, false] - 3.1`)
	be.RunBytecode(bytecode)
	if err = checkCurrentResult(be, []float64{-3.1, -2.1, -3.1}); err != nil {
		t.Error(err)
	}

	bytecode, _, _ = bytefeeder.CompileSource(`[true, false, true] or [false, true, false]`)
	be.RunBytecode(bytecode)
	...
	bytecode, _, _ = bytefeeder.CompileSource(`[1, 2, 3] ** 3`)
	be.RunBytecode(bytecode)
	...
	bytecode, _, _ = bytefeeder.CompileSource(`[1.0, 2.0, 3.0] / [1.0, 2.0, 3.0]`)
	be.RunBytecode(bytecode)
	if err = checkCurrentResult(be, []float64{1.0, 1.0, 1.0}); err != nil {
		t.Error(err)
	}

	bytecode, _, _ = bytefeeder.CompileSource(`["hello", "foo", "this is a string"] + "!"`)
	be.RunBytecode(bytecode)
	if err = checkCurrentResult(be, []string{"hello!", "foo!", "this is a string!"}); err != nil {
		t.Error(err)
	}

	bytecode, _, _ = bytefeeder.CompileSource(`["hello", "foo", "this is a string"] + [" ", " ", " "] + ["world", "bar", "and this is another string"]`)
	be.RunBytecode(bytecode)
