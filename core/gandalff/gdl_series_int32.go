			...
			if o.Len() == 1 {
				if s.isNullable {
					if o.isNullable {
						...
						resultNullMask := __binVecInit(resultSize)
						__binVecOrSS(s.nullMask, o.nullMask, resultNullMask)
						result[0] = int64(s.data[0]) % int64(o.data[0])
						return SeriesInt64{isNullable: true, name: s.name, nullMask: resultNullMask, data: result}
					} else {
						resultSize := len(o.data)
						result := make([]int64, resultSize)
				...
				if s.isNullable {
					if o.isNullable {
						...
						__binVecOrSV(s.nullMask, o.nullMask, resultNullMask)
						for i := 0; i < resultSize; i++ {
							result[i] = int64(s.data[0]) % int64(o.data[i])
						}
						return SeriesInt64{isNullable: true, name: s.name, nullMask: resultNullMask, data: result}
					} else {
						resultSize := len(o.data)
			...
			if o.Len() == 1 {
				if s.isNullable {
					if o.isNullable {
						...
						__binVecOrVS(s.nullMask, o.nullMask, resultNullMask)
						for i := 0; i < resultSize; i++ {
							result[i] = int64(s.data[i]) % int64(o.data[0])
						}
						return SeriesInt64{isNullable: true, name: s.name, nullMask: resultNullMask, data: result}
					} else {
						resultSize := len(s.data)
				...
				if s.isNullable {
					if o.isNullable {
						...
						__binVecOrVV(s.nullMask, o.nullMask, resultNullMask)
						for i := 0; i < resultSize; i++ {
							result[i] = int64(s.data[i]) % int64(o.data[i])
						}
						return SeriesInt64{isNullable: true, name: s.name, nullMask: resultNullMask, data: result}
					} else {
						resultSize := len(s.data)
			...
			if o.Len() == 1 {
				if s.isNullable {
					if o.isNullable {
						...
						resultNullMask := __binVecInit(resultSize)
						__binVecOrSS(s.nullMask, o.nullMask, resultNullMask)
						result[0] = int32(math.Pow(float64(s.data[0]), float64(o.data[0])))
						return SeriesInt32{isNullable: true, name: s.name, nullMask: resultNullMask, data: result}
					} else {
						resultSize := len(o.data)
						result := make([]int32, resultSize)
				...
				if s.isNullable {
					if o.isNullable {
						...
						__binVecOrSV(s.nullMask, o.nullMask, resultNullMask)
						for i := 0; i < resultSize; i++ {
							result[i] = int32(math.Pow(float64(s.data[0]), float64(o.data[i])))
						}
						return SeriesInt32{isNullable: true, name: s.name, nullMask: resultNullMask, data: result}
					} else {
						resultSize := len(o.data)
			...
			if o.Len() == 1 {
				if s.isNullable {
					if o.isNullable {
						...
						__binVecOrVS(s.nullMask, o.nullMask, resultNullMask)
						for i := 0; i < resultSize; i++ {
							result[i] = int32(math.Pow(float64(s.data[i]), float64(o.data[0])))
						}
						return SeriesInt32{isNullable: true, name: s.name, nullMask: resultNullMask, data: result}
					} else {
						resultSize := len(s.data)
				...
				if s.isNullable {
					if o.isNullable {
						...
						__binVecOrVV(s.nullMask, o.nullMask, resultNullMask)
						for i := 0; i < resultSize; i++ {
							result[i] = int32(math.Pow(float64(s.data[i]), float64(o.data[i])))
						}
						return SeriesInt32{isNullable: true, name: s.name, nullMask: resultNullMask, data: result}
					} else {
						resultSize := len(s.data)
