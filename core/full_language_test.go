...
func Test_Assignements(t *testing.T) {
	...
	if be.__currentResult == nil {
		...
		t.Error("Expected float scalar, got", be.__currentResult)
	} else if f, err := be.__currentResult.getFloatScalar(); err != nil || f != 1.0000000603000065 {
		t.Error("Expected 1.0000000603000065, got", f, err)
	}
}
